...
app.post('/api/activate-key', (req, res) => {
    ...
    if (ipAttempts[ip] && ipAttempts[ip].blockUntil > now) {
        console.log(`🟡 [БЛОК] IP: ${ip} временно заблокирован. Время: ${dateStr}`);
    ...
    if (keysDatabase.hasOwnProperty(key)) {
        ...
        if (activeKeys.hasOwnProperty(key)) {
            // Ключ уже был активирован. Тот ли это телефон?
            if (activeKeys[key].deviceId === deviceId) {
                ...
                // Если номера устройств не совпадают
                console.log(`⛔ [ПОПЫТКА КРАЖИ] Ключ ${key} уже привязан к другому устройству! Вор: ${deviceId}`);
                return res.json({ valid: false, expiresAt: 0 });
            }
        } else {
            ...
            // Намертво сохраняем связку "Ключ + Телефон"
            activeKeys[key] = {
                deviceId: deviceId,
                expiresAt: expiresAt
            };
    ...
    console.log(`🔴 [ОШИБКА] Неверный ключ: ${key} | Устройство: ${deviceId} | Время: ${dateStr}`);
    return res.json({ valid: false, expiresAt: 0 });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`Сервер с жесткой привязкой запущен на порту ${PORT}`));